...
def full_name____short(name):
    """Gonzales, Roberto G. -> Gonzales, RG"""
    last, first = name.split(',')
    abbr = ''.join([x[0] for x in first.split(' ')])
    return f'{last}, {abbr}'


def compare(name, fullname):
    last, etc = fullname.split(',', 1)
    if last != name.split(',')[0]:
        return False
    if etc and etc[0] != name.split(',')[1][0]:
        return False
    return True
...
if __name__ == '__main__':
    ...
    wos_base ='D:\\upwork\\Alec-KnowKnow\\developbranch\\variables\\sociology-wos'
